...
function M.only_explorer()
  local sidebar_ft = { explorer = true, settings = true, mason = true }
...
-- Tras renombrar una CARPETA: reapuntar los buffers abiertos que colgaban de ella y,
...
function M.create()
  ...
  rel = (rel ~= "" and rel .. "/" or "")
  local prompt = "Crear en " .. (rel ~= "" and rel or ".") .. " → "
  ...
  vim.ui.input({ prompt = prompt, completion = "dir" }, function(input)
    ...
    local path = vim.fn.fnamemodify(
      -- detectar si input es absoluto o relativo
      input:sub(1, 1) == "/" and input or rel .. input,
      ":p"
    ) -- resolver (relativo al cwd) a absoluto
...
function M.rename()
  ...
  local prompt = "Renombrar '" .. n.name .. "'"
  vim.ui.input({ prompt = prompt, default = n.name, completion = "file" }, function(input)
    if not input or input == "" then
      return
    end -- cancelado o vacío
    if input == n.name then
      return
    end -- sin cambios
    if input:find("[/\\]") then -- no permitir mover con / o \
      vim.notify("Nuevo nombre no puede llevar '/' o '\\'", vim.log.levels.WARN)
      return
    end
    if input:find("^%.%.") then -- no permitir salir con ../
      vim.notify("Ruta no permitida", vim.log.levels.WARN)
      return
    end

    local dir = rel_cwd(vim.fn.fnamemodify(n.path, ":h"))
    local target = vim.fn.fnamemodify((dir ~= "" and dir .. "/" or "") .. input, ":p")
    if target == vim.fn.fnamemodify(n.path, ":p") then
      return
    end -- mismo archivo
    if vim.fn.filereadable(target) == 1 or vim.fn.isdirectory(target) == 1 then
      vim.notify("Ya existe: " .. input, vim.log.levels.WARN)
      return
    end

    if vim.fn.rename(n.path, target) == 0 then
      if n.is_dir then
        rename_dir_buffers(n.path, target)
      else
