...
-- No redimensionable con el mouse: se repone el ancho. Con DEBOUNCE (al soltar el
-- separador), porque corregir en cada evento pelea con el arrastre en curso y colapsa.
local fixw_timer
...
-- Paneles fijos: marcarlos para que whichkey bloquee los <C-w> que MUEVEN la ventana
-- (H/J/K/L, swap x, rotar r/R, a otra tab T). No sirve un keymap: whichkey reenvía el
-- comando nativo con noremap. Los <C-w> de NAVEGACIÓN (h/l/w/p…) siguen funcionando.
api.nvim_create_autocmd("FileType", {
  group = api.nvim_create_augroup("PanelFixed", { clear = true }),
  pattern = { "explorer", "settings", "mason", "minimap" },
  callback = function(ev)
    vim.b[ev.buf].fixed_panel = true
  end,
})
